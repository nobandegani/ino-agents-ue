...
// ============================================================================
//
// End-to-end smoke test for the UInoChatterboxTurboNativeStreamSynthesize async
// action — mirrors Ino.Chatterbox.SubsystemSynthTest but uses the
// streaming path instead of the all-at-end SynthesizeAsync.
//
// Flow:
//   1. Resolve UInoChatterboxTurboNativeSubsystem via the first active world
//      (same helper the non-streaming subsystem test uses).
//   2. LoadModelsAsync the requested variant.
//   3. On load success, construct a UInoChatterboxTurboNativeStreamSynthesize via
...
#include "InoChatterboxTurboNativeStreamSynthTest.h"
...
void UInoChatterboxTurboNativeStreamSynthTestObserver::HandleLoaded(
    bool bSuccess, FString ErrorMessage)
{
    ...
    Action = UInoChatterboxTurboNativeStreamSynthesize::StreamSynthesize(
    ...
    Action->OnAudioChunk.AddDynamic(
        this, &UInoChatterboxTurboNativeStreamSynthTestObserver::HandleChunk);
    Action->OnComplete.AddDynamic(
        this, &UInoChatterboxTurboNativeStreamSynthTestObserver::HandleComplete);
    Action->OnError.AddDynamic(
        this, &UInoChatterboxTurboNativeStreamSynthTestObserver::HandleError);
...
void UInoChatterboxTurboNativeStreamSynthTestObserver::HandleChunk(
...
void UInoChatterboxTurboNativeStreamSynthTestObserver::HandleComplete(
    bool                          bSuccess,
...
void UInoChatterboxTurboNativeStreamSynthTestObserver::HandleError(FString ErrorMessage)
{
    UE_LOG(LogInoAgents, Error,
...
void UInoChatterboxTurboNativeStreamSynthTestObserver::Teardown()
{
    if (Subsystem)
    {
...
namespace
{
    void RunStreamSynthTest(const TArray<FString>& Args)
    {
        ...
        {
            UE_LOG(LogInoAgents, Error,
                   TEXT("StreamSynthTest: no UInoChatterboxTurboNativeSubsystem — run from PIE"));
            return;
        }
        ...
        auto* Observer = NewObject<UInoChatterboxTurboNativeStreamSynthTestObserver>();
        ...
        OnLoaded.BindDynamic(Observer, &UInoChatterboxTurboNativeStreamSynthTestObserver::HandleLoaded);
    ...
    FAutoConsoleCommand GChatterboxStreamSynthTestCmd(
        ...
        TEXT("UInoChatterboxTurboNativeStreamSynthesize end-to-end — LoadModelsAsync, ")
