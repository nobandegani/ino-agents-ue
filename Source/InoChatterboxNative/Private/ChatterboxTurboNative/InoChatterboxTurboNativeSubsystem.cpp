...
#include "InoChatterboxTurboNativeSynthesisWorker.h"
...
UInoChatterboxTurboNativeSubsystem::UInoChatterboxTurboNativeSubsystem() = default;

UInoChatterboxTurboNativeSubsystem::UInoChatterboxTurboNativeSubsystem(FVTableHelper& Helper)
    : Super(Helper)
{
}
...
void UInoChatterboxTurboNativeSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
    ...
    Models.Reset();
    Tokenizer.Reset();
    LoadedVariant  = EInoChatterboxTurboNativeVariant::Q4F16;
    bLoadInFlight  = false;
...
void UInoChatterboxTurboNativeSubsystem::Deinitialize()
{
...
void UInoChatterboxTurboNativeSubsystem::LoadModelsAsync(
    const FInoChatterboxTurboNativeModelConfig&        Config,
    const FOnInoChatterboxTurboNativeDownloadProgress& OnDownloadProgress,
    const FOnInoChatterboxTurboNativeModelsLoaded&     OnLoaded)
{
    ...
    EInoChatterboxTurboNativeVariant EncV, EmbedV, LMV, DecV;
...
void UInoChatterboxTurboNativeSubsystem::DispatchLoadWorker(
    const FInoChatterboxTurboNativeModelConfig& Config)
{
    check(IsInGameThread());
    check(bLoadInFlight);
    ...
    TWeakObjectPtr<UInoChatterboxTurboNativeSubsystem> WeakThis(this);
    const FOnInoChatterboxTurboNativeModelsLoaded         OnLoaded   = PendingOnLoaded;
    ...
    const FInoChatterboxTurboNativePerformanceOptions     Performance = Config.Performance;
    ...
    FInoChatterboxTurboNativeModels::FSessionLoadSpec EncSpec   { ChatterboxResolveVariantDir(EncV),   ChatterboxVariantToString(EncV)   };
    FInoChatterboxTurboNativeModels::FSessionLoadSpec EmbedSpec { ChatterboxResolveVariantDir(EmbedV), ChatterboxVariantToString(EmbedV) };
    FInoChatterboxTurboNativeModels::FSessionLoadSpec LMSpec    { ChatterboxResolveVariantDir(LMV),    ChatterboxVariantToString(LMV)    };
    FInoChatterboxTurboNativeModels::FSessionLoadSpec DecSpec   { ChatterboxResolveVariantDir(DecV),   ChatterboxVariantToString(DecV)   };
    ...
    const EInoChatterboxTurboNativeVariant RepresentativeVariant = EncV;
           ...
           WeakThis, OnLoaded, Performance, TStart]()
    {
        ...
        TUniquePtr<FInoChatterboxTurboNativeTokenizer> LocalTokenizer =
            FInoChatterboxTurboNativeTokenizer::LoadFromJson(TokenizerPath, &LocalError);

        TUniquePtr<FInoChatterboxTurboNativeModels> LocalModels;
        if (LocalTokenizer.IsValid())
        {
            LocalModels = FInoChatterboxTurboNativeModels::LoadPerSession(
             ...
             LocalModels = MoveTemp(LocalModels)]() mutable
        {
            ...
            UInoChatterboxTurboNativeSubsystem* Subsys = WeakThis.Get();
            ...
            Subsys->Worker = MakeUnique<FInoChatterboxTurboNativeSynthesisWorker>(
...
void UInoChatterboxTurboNativeSubsystem::UnloadModels()
{
    check(IsInGameThread());
    ...
    if (bWasDownloading)
    {
        ...
        const FOnInoChatterboxTurboNativeModelsLoaded OnLoadedCopy = PendingOnLoaded;
        CleanupDownload();
        bLoadInFlight  = false;
        bPendingUnload = false;
...
bool UInoChatterboxTurboNativeSubsystem::IsModelsLoaded() const
{
...
namespace
{
    ...
    FString ChatterboxComponentFilename(const TCHAR* Component, EInoChatterboxTurboNativeVariant Variant)
    {
    ...
    FString ChatterboxComponentDataFilename(const TCHAR* Component, EInoChatterboxTurboNativeVariant Variant)
    {
...
bool UInoChatterboxTurboNativeSubsystem::IsModelDownloaded(EInoChatterboxTurboNativeVariant Variant) const
{
    // Required file set for a runnable variant. Matches
    // FInoChatterboxTurboNativeModels::LoadFromDir's file-discovery logic.
    //
...
bool UInoChatterboxTurboNativeSubsystem::IsConfigDownloaded(
    const FInoChatterboxTurboNativeModelConfig& Config) const
{
    ...
    struct FCheck
    {
        ...
        EInoChatterboxTurboNativeVariant Variant;
    };
    const FCheck Checks[] = {
    ...
    const EInoChatterboxTurboNativeVariant TokDirs[] = { EncV, EmbedV, LMV, DecV };
    for (const EInoChatterboxTurboNativeVariant V : TokDirs)
    {
...
EInoChatterboxTurboNativeVariant UInoChatterboxTurboNativeSubsystem::GetLoadedVariant() const
{
    UE_LOG(LogInoAgents, Verbose,
...
void UInoChatterboxTurboNativeSubsystem::SynthesizeAsync(
    const FString& Text,
    const FInoChatterboxTurboNativeVoice& Voice,
    const FInoChatterboxTurboNativeSynthesisOptions& Options,
    const FOnInoChatterboxTurboNativeSynthesisComplete& OnComplete)
{
    UE_LOG(LogInoAgents, Log,
    ...
    EnqueueSynth(Text, Voice, Options,
                 /*StreamChunkTokens=*/ 0,
                 FOnInoChatterboxTurboNativeAudioChunk(),
                 OnComplete);
}

void UInoChatterboxTurboNativeSubsystem::SynthesizeStreamAsync(
    const FString& Text,
    const FInoChatterboxTurboNativeVoice& Voice,
    const FInoChatterboxTurboNativeSynthesisOptions& Options,
    const FOnInoChatterboxTurboNativeAudioChunk& OnAudioChunk,
    const FOnInoChatterboxTurboNativeSynthesisComplete& OnComplete,
...
void UInoChatterboxTurboNativeSubsystem::EnqueueSynth(
    const FString& Text,
    const FInoChatterboxTurboNativeVoice& Voice,
    const FInoChatterboxTurboNativeSynthesisOptions& Options,
    int32 StreamChunkTokens,
    ...
    const FOnInoChatterboxTurboNativeSynthesisComplete& OnComplete)
{
    ...
    auto FailNow = [&OnComplete](const FString& Msg)
    {
        ...
        FInoChatterboxTurboNativeSynthesisResult Empty;
    ...
    //
    // Four sources, priority order (matches FInoChatterboxTurboNativeVoice doc):
    ...
    //      the caller passes an empty FInoChatterboxTurboNativeVoice. Makes the
    ...
    if (!ResolvedWavPath.IsEmpty())
    {
        if (bUsingDefaultVoice && !IFileManager::Get().FileExists(*ResolvedWavPath))
        {
            FailNow(FString::Printf(
                ...
                TEXT("you may need to either pass an explicit FInoChatterboxTurboNativeVoice ")
    ...
    FInoChatterboxTurboNativeSynthesisWorker::FPendingSynth Item;
    Item.Text              = Text;
...
void UInoChatterboxTurboNativeSubsystem::CancelSynthesis()
{
    check(IsInGameThread());
...
namespace
{
    ...
    void BuildRepoBaseUrl(
        const FInoChatterboxTurboNativeModelEntry& Entry,
        FString&                        OutBase)
    {
    ...
    void AppendSessionFiles(
        TArray<FInoChatterboxTurboNativeDownloadFile>& Queue,
        const FString&                      Base,
        const TCHAR*                        Component,
        EInoChatterboxTurboNativeVariant               Variant,
        const FString&                      TargetDir)
    {
        using FFile = FInoChatterboxTurboNativeDownloadFile;
     ...
     *  variants. */
    TArray<FInoChatterboxTurboNativeDownloadFile> BuildDownloadQueue(
        const UInoChatterboxNativeSettings*        Settings,
        const FInoChatterboxTurboNativeModelConfig& Config)
    {
        TArray<FInoChatterboxTurboNativeDownloadFile> Queue;
        if (Settings == nullptr)
        ...
        // has one.
        const FInoChatterboxTurboNativeModelEntry* SharedEntry =
            Settings->FindChatterboxModel(EncV);
        EInoChatterboxTurboNativeVariant SharedVariant = EncV;
        if (SharedEntry == nullptr)
        {
            for (const FSessionSpec& S : Sessions)
            {
                if (const FInoChatterboxTurboNativeModelEntry* E =
                        Settings->FindChatterboxModel(S.Variant))
                {
                    SharedEntry   = E;
...
void UInoChatterboxTurboNativeSubsystem::StartDownload()
{
    check(IsInGameThread());
    check(bLoadInFlight);
    ...
    const UInoChatterboxNativeSettings* Settings = UInoChatterboxNativeSettings::Get();
    if (Settings == nullptr)
    {
        FinishDownloadError(
            TEXT("UInoChatterboxNativeSettings unavailable."));
        return;
    }
    ...
    const EInoChatterboxTurboNativeVariant ProbeVariants[] = { EncV, EmbedV, LMV, DecV };
    bool bAnyEntry = false;
    for (const EInoChatterboxTurboNativeVariant V : ProbeVariants)
    {
...
void UInoChatterboxTurboNativeSubsystem::StartHeadProbe()
{
    check(IsInGameThread());
    ...
    if (DownloadCursor >= DownloadQueue.Num())
    {
        ...
        int32 KnownSizes = 0;
        int64 KnownTotal = 0;
        for (const FInoChatterboxTurboNativeDownloadFile& F : DownloadQueue)
        {
    ...
    const FInoChatterboxTurboNativeDownloadFile& File = DownloadQueue[DownloadCursor];
    ...
    DownloadRequest->OnProcessRequestComplete().BindUObject(
        this, &UInoChatterboxTurboNativeSubsystem::HandleHeadComplete);
...
void UInoChatterboxTurboNativeSubsystem::HandleHeadComplete(
    FHttpRequestPtr /*Request*/, FHttpResponsePtr Response, bool bSucceeded)
{
    ...
    FInoChatterboxTurboNativeDownloadFile& File = DownloadQueue[DownloadCursor];
...
void UInoChatterboxTurboNativeSubsystem::StartNextFileDownload()
{
    check(IsInGameThread());
    ...
    DownloadRequest->OnHeaderReceived().BindUObject(
        this, &UInoChatterboxTurboNativeSubsystem::HandleDownloadHeader);
    ...
    DownloadRequest->OnRequestProgress64().BindUObject(
        this, &UInoChatterboxTurboNativeSubsystem::HandleDownloadProgress);
    DownloadRequest->OnProcessRequestComplete().BindUObject(
        this, &UInoChatterboxTurboNativeSubsystem::HandleDownloadComplete);

    DownloadRequest->ProcessRequest();
}

void UInoChatterboxTurboNativeSubsystem::HandleDownloadProgress(
...
void UInoChatterboxTurboNativeSubsystem::HandleDownloadHeader(
...
void UInoChatterboxTurboNativeSubsystem::HandleDownloadComplete(
    FHttpRequestPtr /*Request*/, FHttpResponsePtr Response, bool bSucceeded)
{
    check(IsInGameThread());
    DownloadRequest.Reset();
...
void UInoChatterboxTurboNativeSubsystem::BroadcastDownloadProgress()
{
    ...
    {
        const FInoChatterboxTurboNativeDownloadFile& F = DownloadQueue[i];
    ...
    else
    {
        ...
        {
            const FInoChatterboxTurboNativeDownloadFile& Current = DownloadQueue[DownloadCursor];
...
void UInoChatterboxTurboNativeSubsystem::FinishDownloadSuccess()
{
    check(IsInGameThread());
    ...
    const FInoChatterboxTurboNativeModelConfig Config = PendingConfig;
...
void UInoChatterboxTurboNativeSubsystem::FinishDownloadError(const FString& Err)
{
    check(IsInGameThread());
...
void UInoChatterboxTurboNativeSubsystem::CleanupDownload()
{
