...
namespace
{
    ...
    TArray<float> TranscodeRAW(
        const uint8*              Bytes,
        int32                     NumBytes,
        EInoAgentsRAWAudioFormat  Format)
    {
        ...
        switch (Format)
        {
            case EInoAgentsRAWAudioFormat::Int16:   return TranscodeInt16  (Bytes, NumBytes);
            case EInoAgentsRAWAudioFormat::Int32:   return TranscodeInt32  (Bytes, NumBytes);
            case EInoAgentsRAWAudioFormat::UInt8:   return TranscodeUInt8  (Bytes, NumBytes);
            case EInoAgentsRAWAudioFormat::Float32: return TranscodeFloat32(Bytes, NumBytes);
        }
        return {};
    }
}
...
UInoAgentsStreamingSoundWave::UInoAgentsStreamingSoundWave(
...
void UInoAgentsStreamingSoundWave::BeginDestroy()
{
...
UInoAgentsStreamingSoundWave* UInoAgentsStreamingSoundWave::CreateStreamingSoundWave()
{
    // Transient — never saved to disk. NewObject is fine for a
    // runtime-only asset; no DefaultSubobject quirks apply here.
    return NewObject<UInoAgentsStreamingSoundWave>();
}
...
bool UInoAgentsStreamingSoundWave::ResolveStreamFormat(
...
void UInoAgentsStreamingSoundWave::AppendAudioDataFromRAW(
    const TArray<uint8>&      RAWData,
    EInoAgentsRAWAudioFormat  RAWFormat,
    int32                     InSampleRate,
    int32                     InNumChannels)
{
    if (RAWData.Num() == 0)
...
void UInoAgentsStreamingSoundWave::AppendAudioDataFromMP3(
    const TArray<uint8>& MP3Data)
{
    ...
    AudioTaskPipe->Launch(TEXT("AppendAudioDataFromMP3"),
        [WeakThis, OwnedBytes = MoveTemp(OwnedBytes)]() mutable
        {
            ...
            {
                Self->Mp3State = InoAgentsAudioMp3::CreateState();
            }

            FInoAgentsMp3DecodeResult Decoded =
                InoAgentsAudioMp3::Feed(*Self->Mp3State, OwnedBytes);
...
void UInoAgentsStreamingSoundWave::PreAllocateAudioData(
    int64 NumOfBytesToPreAllocate,
    const FOnInoAgentsPreAllocateAudioDataResult& Result)
{
    FOnInoAgentsPreAllocateAudioDataResultNative Native;
    Native.BindLambda(
...
void UInoAgentsStreamingSoundWave::PreAllocateAudioData(
    int64 NumOfBytesToPreAllocate,
    const FOnInoAgentsPreAllocateAudioDataResultNative& Result)
{
    if (!AudioTaskPipe.IsValid())
    {
...
void UInoAgentsStreamingSoundWave::ResetStreamingBuffer()
{
    if (!AudioTaskPipe.IsValid())
    {
        return;
...
void UInoAgentsStreamingSoundWave::BroadcastAudioError(const FString& Message)
{
    UE_LOG(LogInoAgents, Error, TEXT("UInoAgentsStreamingSoundWave: %s"), *Message);
