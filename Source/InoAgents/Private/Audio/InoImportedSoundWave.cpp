...
UInoImportedSoundWave::UInoImportedSoundWave(const FObjectInitializer& ObjectInitializer)
...
UInoImportedSoundWave* UInoImportedSoundWave::CreateImportedSoundWave()
{
    return NewObject<UInoImportedSoundWave>();
}
...
void UInoImportedSoundWave::SetInitialDesiredSampleRate(int32 InSampleRate)
{
    const uint32 Clamped = static_cast<uint32>(FMath::Clamp(InSampleRate, 8000, 192000));
    SetSampleRate(Clamped);
}

void UInoImportedSoundWave::SetInitialDesiredNumChannels(int32 InNumChannels)
{
    NumChannels = FMath::Clamp(InNumChannels, 1, 2);
}

int32 UInoImportedSoundWave::GetSampleRate() const
{
    return static_cast<int32>(GetSampleRateForCurrentPlatform());
}

int32 UInoImportedSoundWave::GetNumOfChannels() const
{
    return NumChannels;
}
...
float UInoImportedSoundWave::GetPlaybackTime() const
{
...
float UInoImportedSoundWave::GetDurationSeconds() const
{
    const int32 Rate = FMath::Max<int32>(GetSampleRate(), 1);
    FScopeLock Lock(&SharedPCM->Guard);
...
int64 UInoImportedSoundWave::GetPlayedFrames() const
{
    FScopeLock Lock(&SharedPCM->Guard);
    return PlayedFrames;
}

int64 UInoImportedSoundWave::GetTotalFrames() const
{
    FScopeLock Lock(&SharedPCM->Guard);
    return SharedPCM->TotalFrames;
}

float UInoImportedSoundWave::GetPlaybackPercentage() const
{
    FScopeLock Lock(&SharedPCM->Guard);
...
bool UInoImportedSoundWave::IsPlaybackFinished() const
{
    FScopeLock Lock(&SharedPCM->Guard);
    return SharedPCM->TotalFrames > 0 && PlayedFrames >= SharedPCM->TotalFrames;
}

FInoAudioHeaderInfo UInoImportedSoundWave::GetAudioHeaderInfo() const
{
    FInoAudioHeaderInfo Info;
...
void UInoImportedSoundWave::RewindPlaybackTime(float StartTime)
{
    const int32 Rate = FMath::Max<int32>(GetSampleRate(), 1);
...
void UInoImportedSoundWave::ReleasePlayedAudioData()
{
    FScopeLock Lock(&SharedPCM->Guard);
...
void UInoImportedSoundWave::ReleaseMemory()
{
    {
        FScopeLock Lock(&SharedPCM->Guard);
...
void UInoImportedSoundWave::SetSoundLooping(bool bLoop)
{
    FScopeLock Lock(&SharedPCM->Guard);
    bLooping = bLoop;
    if (bLoop)
...
void UInoImportedSoundWave::ForceStopPlayback()
{
...
bool UInoImportedSoundWave::ResampleSoundWave(int32 NewSampleRate)
{
...
bool UInoImportedSoundWave::MixSoundWaveChannels(int32 NewNumChannels)
{
...
void UInoImportedSoundWave::ReverseAudioBuffer()
{
    FScopeLock Lock(&SharedPCM->Guard);
...
UInoImportedSoundWave* UInoImportedSoundWave::DuplicateSoundWave(bool bShareBuffer)
{
    UInoImportedSoundWave* Dup =
        NewObject<UInoImportedSoundWave>(GetTransientPackage(), GetClass());
    if (Dup == nullptr)
    {
        return nullptr;
    }
    ...
    else
    {
        ...
        FScopeLock Lock(&SharedPCM->Guard);
        Dup->SharedPCM = MakeShared<FInoSharedPCMBuffer>();
...
TArray<float> UInoImportedSoundWave::GetPCMBuffer() const
{
    FScopeLock Lock(&SharedPCM->Guard);
...
void UInoImportedSoundWave::SetNumSamplesPerChunk(int32 NumSamples)
{
...
void UInoImportedSoundWave::AppendFloat32Frames(
    TArray<float>&& Interleaved, int64 NumFramesInBlock)
{
    ...
    TWeakObjectPtr<UInoImportedSoundWave> WeakThis(this);
    AsyncTask(ENamedThreads::GameThread,
        [WeakThis, Data = MoveTemp(BroadcastCopy), bWantPopulateData]() mutable
        {
            UInoImportedSoundWave* Self = WeakThis.Get();
...
int32 UInoImportedSoundWave::GeneratePCMData(
...
void UInoImportedSoundWave::BroadcastPlaybackFinished()
{
    TWeakObjectPtr<UInoImportedSoundWave> WeakThis(this);
    AsyncTask(ENamedThreads::GameThread,
        [WeakThis]()
