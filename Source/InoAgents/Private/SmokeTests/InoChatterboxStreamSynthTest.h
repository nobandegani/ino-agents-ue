...
#include "InoChatterboxStreamSynthTest.generated.h"

class UInoChatterboxTtsSubsystem;
class UInoChatterboxStreamSynthesize;

/**
 * Observer UCLASS for the Ino.Chatterbox.StreamSynthTest console
 * command. Mirrors UInoChatterboxSubsystemTestObserver but drives the
 * UInoChatterboxStreamSynthesize async action instead of calling
 ...
 * InoChatterboxStreamSynthesize.h exactly.
 */
UCLASS()
class UInoChatterboxStreamSynthTestObserver : public UObject
{
    GENERATED_BODY()
...
public:
    ...
    UPROPERTY()
    TObjectPtr<UInoChatterboxTtsSubsystem> Subsystem = nullptr;
    ...
    UPROPERTY()
    TObjectPtr<UInoChatterboxStreamSynthesize> Action = nullptr;
