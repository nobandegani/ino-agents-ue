...
//
//   InoAgents.SoundWave.StreamingPcm
...
//
//   InoAgents.SoundWave.StreamingMp3 [path]
...
#include "InoAgentsSoundWaveStreamingTest.h"
...
void UInoAgentsSoundWaveStreamingTestObserver::HandlePopulate(const TArray<float>& Data)
{
    ++PopulateFires;
    if (PopulateFires == 1)
    {
...
void UInoAgentsSoundWaveStreamingTestObserver::HandleGeneratePcm(const TArray<float>& Data)
{
    ++GeneratePcmFires;
    GeneratePcmSamples += Data.Num();
}

void UInoAgentsSoundWaveStreamingTestObserver::HandlePlaybackFinished()
{
    const double Elapsed = FPlatformTime::Seconds() - StartTime;
    bFinishedFired = true;
    UE_LOG(LogInoAgents, Log,
...
void UInoAgentsSoundWaveStreamingTestObserver::HandleError(FString ErrorMessage)
{
    const double Elapsed = FPlatformTime::Seconds() - StartTime;
    bErrored = true;
...
void UInoAgentsSoundWaveStreamingTestObserver::Finish()
{
...
namespace
{
     ...
     * detect).
     */
    UInoAgentsSoundWaveStreamingTestObserver* SpawnObserver(
        const FString& TestName, bool bBindGeneratePcm)
    {
        ...
        UInoAgentsStreamingSoundWave* Wave =
            UInoAgentsStreamingSoundWave::CreateStreamingSoundWave();
        Comp->SetSound(Wave);

        UInoAgentsSoundWaveStreamingTestObserver* Observer =
            NewObject<UInoAgentsSoundWaveStreamingTestObserver>();
        ...
        Wave->OnPopulateAudioData.AddDynamic(
            Observer, &UInoAgentsSoundWaveStreamingTestObserver::HandlePopulate);
        Wave->OnAudioPlaybackFinished.AddDynamic(
            Observer, &UInoAgentsSoundWaveStreamingTestObserver::HandlePlaybackFinished);
        Wave->OnAudioError.AddDynamic(
            Observer, &UInoAgentsSoundWaveStreamingTestObserver::HandleError);
        if (bBindGeneratePcm)
        {
            Wave->OnGeneratePCMData.AddDynamic(
                Observer, &UInoAgentsSoundWaveStreamingTestObserver::HandleGeneratePcm);
        }
...
static void RunStreamingPcmTest(const TArray<FString>& /*Args*/)
{
    ...
    Observer->Wave->AppendAudioDataFromRAW(
        PcmBytes, EInoAgentsRAWAudioFormat::Int16, kSineSampleRate, 1);
...
static FAutoConsoleCommand GStreamingPcmCmd(
    TEXT("InoAgents.SoundWave.StreamingPcm"),
    TEXT("Play a 1-second 440 Hz sine wave through UInoAgentsStreamingSoundWave "
    ...
    if (!FFileHelper::LoadFileToArray(FileBytes, *Path))
    {
        UE_LOG(LogInoAgents, Error,
               TEXT("StreamingMp3: could not read %s. Run "
                    "InoAgents.ElevenLabs.DialogueStreamTest first, or pass "
                    "a path: InoAgents.SoundWave.StreamingMp3 <path>"),
               *Path);
        return;
    }
...
static FAutoConsoleCommand GStreamingMp3Cmd(
    TEXT("InoAgents.SoundWave.StreamingMp3"),
    TEXT("Play an MP3 file through UInoAgentsStreamingSoundWave's MP3 append "
...
static void RunStreamingMp3ChunkedTest(const TArray<FString>& Args)
{
    ...
    struct FChunkState
    {
        TWeakObjectPtr<UInoAgentsSoundWaveStreamingTestObserver> ObserverWeak;
        TArray<uint8> Bytes;
        int32         Offset = 0;
    };
    ...
    FTSTicker::GetCoreTicker().AddTicker(
        FTickerDelegate::CreateLambda(
            ...
            {
                UInoAgentsSoundWaveStreamingTestObserver* Obs = State->ObserverWeak.Get();
...
static FAutoConsoleCommand GStreamingMp3ChunkedCmd(
    TEXT("InoAgents.SoundWave.StreamingMp3Chunked"),
...
static FAutoConsoleCommand GVisualizationCmd(
    TEXT("InoAgents.SoundWave.Visualization"),
