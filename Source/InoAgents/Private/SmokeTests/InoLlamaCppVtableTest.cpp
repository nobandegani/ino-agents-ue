 ...
 *
 * Expected output on a healthy b8883 install: "40 OK, 0 MISSING" (or
 * whatever the current total is) with every member logged OK.
 */
...
namespace
{
    ...
    void RunVtableTest(const TArray<FString>& /*Args*/)
    {
        ...
        UE_LOG(LogInoAgents, Log, TEXT("=== Ino.LlamaCpp.VtableTest ==="));

        int32 NumOk = 0;
        int32 NumMissing = 0;

        UE_LOG(LogInoAgents, Log, TEXT("  Milestone C — module startup:"));
        ...
        UE_LOG(LogInoAgents, Log, TEXT("  NeuTTS prereq — model:"));
        ...
        UE_LOG(LogInoAgents, Log, TEXT("  NeuTTS prereq — context:"));
        ...
        UE_LOG(LogInoAgents, Log, TEXT("  NeuTTS prereq — memory / KV-cache:"));
        INO_REPORT_MEMBER(llama_get_memory);
        INO_REPORT_MEMBER(llama_memory_clear);

        UE_LOG(LogInoAgents, Log, TEXT("  NeuTTS prereq — vocab:"));
        ...
        UE_LOG(LogInoAgents, Log, TEXT("  NeuTTS prereq — tokenize / detokenize:"));
        INO_REPORT_MEMBER(llama_tokenize);
        INO_REPORT_MEMBER(llama_detokenize);

        UE_LOG(LogInoAgents, Log, TEXT("  NeuTTS prereq — batch + decode:"));
        ...
        UE_LOG(LogInoAgents, Log, TEXT("  NeuTTS prereq — sampler chain:"));
