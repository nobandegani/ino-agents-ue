...
// Invoke:
//     Ino.ToolCallTest
//     Ino.ToolCallTest What is 100 plus 250?
// ============================================================================
...
static void RunToolCallSmokeTest(const TArray<FString>& Args)
{
    ...
    const TSharedPtr<FJsonObject> Response1Obj =
        InoSmokeTest::ParseJsonObjectOrLog(Response1Json, TEXT("ToolCallTest round 1"));
    ...
    const bool bHasToolCall =
        InoSmokeTest::TryExtractFirstToolCall(Response1Obj, ToolName, ArgumentsObj);

    if (!bHasToolCall)
    {
        const FString AssistantText = InoSmokeTest::ExtractAssistantText(Response1Obj);
        UE_LOG(LogInoAgents, Warning,
    ...
    const TSharedPtr<FJsonObject> Response2Obj =
        InoSmokeTest::ParseJsonObjectOrLog(Response2Json, TEXT("ToolCallTest round 2"));
    if (Response2Obj.IsValid())
    {
        const FString FinalText = InoSmokeTest::ExtractAssistantText(Response2Obj);
...
static FAutoConsoleCommand GToolCallTestCommand(
    ...
    TEXT("Phase-1 smoke test (milestone B): full agent loop. Registers an "
         ...
         "takes a custom prompt: 'Ino.ToolCallTest What is 100 plus 250?'"),
