...
/**
 * Console command for validating the Milestone C llama.cpp module-startup
 ...
 *   one of:
 *     - Plugins/InoAgents/LlamaCpp/scripts/setup-llamacpp.ps1 not run
...
namespace
{
    void RunBackendInfoTest(const TArray<FString>& /*Args*/)
    {
        ...
        UE_LOG(LogInoAgents, Log, TEXT("=== Ino.LlamaCpp.BackendInfoTest ==="));
        ...
        if (NumBackends == 0)
        {
            UE_LOG(LogInoAgents, Warning,
                   ...
                   TEXT("in Binaries/ThirdParty/InoLlamaCpp/<platform>/ ?"));
            return;
        }
        ...
        for (size_t i = 0; i < NumBackends; ++i)
        {
            ...
            if (Reg == nullptr)
            {
                UE_LOG(LogInoAgents, Warning, TEXT("    [%llu] (null reg)"),
        ...
        UE_LOG(LogInoAgents, Log, TEXT("=== BackendInfoTest complete ==="));
    }
