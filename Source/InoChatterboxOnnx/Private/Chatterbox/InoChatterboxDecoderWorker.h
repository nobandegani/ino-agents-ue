...
/**
 * FInoChatterboxDecoderWorker — a single-thread "latest request wins"
 ...
 *
 * Ownership / lifecycle:
 *   - Constructed by FInoChatterboxRunner::SynthesizeText when
 ...
 *   - Destructor (~FInoChatterboxDecoderWorker) signals stop,
 ...
 *   existing FInoChatterboxSynthesisWorker.
 */
class FInoChatterboxDecoderWorker : public FRunnable
{
public:
    /**
     ...
     */
    explicit FInoChatterboxDecoderWorker(FDecodeTask InTask);

    /**
     * Signals stop, joins the thread, returns FEvents. Blocking;
     * bounded by one decoder Run() (typically tens to hundreds of ms).
     */
    virtual ~FInoChatterboxDecoderWorker();

    FInoChatterboxDecoderWorker(const FInoChatterboxDecoderWorker&) = delete;
    FInoChatterboxDecoderWorker& operator=(const FInoChatterboxDecoderWorker&) = delete;
