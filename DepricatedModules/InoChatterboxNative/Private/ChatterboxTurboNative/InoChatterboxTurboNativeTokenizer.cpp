...
// ============================================================================
//  FInoChatterboxTurboNativeTokenizer — loading
// ============================================================================

TUniquePtr<FInoChatterboxTurboNativeTokenizer> FInoChatterboxTurboNativeTokenizer::LoadFromJson(
    ...
    FString* OutError)
{
    auto Fail = [OutError](FString Msg)
    {
        ...
        return TUniquePtr<FInoChatterboxTurboNativeTokenizer>(nullptr);
    };
    ...
    TUniquePtr<FInoChatterboxTurboNativeTokenizer> Tk(new FInoChatterboxTurboNativeTokenizer());
...
TArray<FString> FInoChatterboxTurboNativeTokenizer::BpeEncode(const FString& PreToken) const
{
...
TArray<int64> FInoChatterboxTurboNativeTokenizer::EncodeNonSpecialSegment(const FString& Segment) const
{
    TArray<int64> Ids;
    if (Segment.IsEmpty())
    {
...
TArray<int64> FInoChatterboxTurboNativeTokenizer::Encode(const FString& Text, bool bAddSpecialTokens) const
{
    TArray<int64> Ids;
...
FString FInoChatterboxTurboNativeTokenizer::Decode(TArrayView<const int64> Ids) const
{
    FString ByteLevel;
...
int64 FInoChatterboxTurboNativeTokenizer::GetSpecialTokenId(const FString& Content) const
{
    const int64* Id = SpecialTokenIds.Find(Content);
    return Id ? *Id : INT64_C(-1);
}

int32 FInoChatterboxTurboNativeTokenizer::GetTotalVocabSize() const
{
    return VocabIdToString.Num();
}

void FInoChatterboxTurboNativeTokenizer::LogSummary() const
{
    UE_LOG(LogInoAgents, Log,
