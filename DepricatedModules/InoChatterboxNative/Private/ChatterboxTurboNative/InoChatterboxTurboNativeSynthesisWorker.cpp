...
FInoChatterboxTurboNativeSynthesisWorker::FInoChatterboxTurboNativeSynthesisWorker(
    ...
    , Tokenizer(InTokenizer)
{
    ...
    Thread.Reset(FRunnableThread::Create(
        this,
        TEXT("InoChatterboxTurboNativeSynthesisWorker"),
        /*InStackSize=*/ 0,
        TPri_Normal));
...
void FInoChatterboxTurboNativeSynthesisWorker::Enqueue(FPendingSynth Item)
{
    check(IsInGameThread());
...
void FInoChatterboxTurboNativeSynthesisWorker::CancelAndFlush()
{
    check(IsInGameThread());
...
uint32 FInoChatterboxTurboNativeSynthesisWorker::Run()
{
...
void FInoChatterboxTurboNativeSynthesisWorker::Stop()
{
...
void FInoChatterboxTurboNativeSynthesisWorker::ProcessSynth(FPendingSynth& Item)
{
    ...
    FInoChatterboxTurboNativeRunner Runner(Models, Tokenizer);

    FInoChatterboxTurboNativeRunner::FSynthesisOptions RunnerOpts;
    ...
    const FOnInoChatterboxTurboNativeAudioChunk OnAudioChunkCopy = Item.OnAudioChunk;

    FInoChatterboxTurboNativeRunner::FOnStreamChunk StreamCb;
    if (bStreaming)
    {
    ...
    FInoChatterboxTurboNativeRunner::FSynthesisResult NativeResult;
    FString NativeError;
    ...
    // for the Blueprint surface. Matches FInoChatterboxTurboNativeSynthesisResult::
    // AudioSamples's documented format. A 1-2 s utterance quantizes in
    // well under a millisecond on a worker thread.
    FInoChatterboxTurboNativeSynthesisResult BpResult;
    ...
    // UInoChatterboxTurboNativeSubsystem::GetOutputSampleRate() instead (single
    ...
    const FOnInoChatterboxTurboNativeSynthesisComplete OnCompleteCopy = Item.OnComplete;
    AsyncTask(ENamedThreads::GameThread,
