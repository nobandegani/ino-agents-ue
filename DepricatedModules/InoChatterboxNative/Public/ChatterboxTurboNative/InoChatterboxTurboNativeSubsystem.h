...
#include "InoChatterboxTurboNativeSubsystem.generated.h"
...
// InoChatterboxTurboNativeSubsystem.cpp. Leaving any of them implicit produces
...
class FInoChatterboxTurboNativeSynthesisWorker;
 ...
 * UInoChatterboxTurboNativeSubsystem) so the build-queue helper in the
 ...
 */
struct FInoChatterboxTurboNativeDownloadFile
{
 ...
 *     UInoChatterboxTurboNativeSubsystem* Subsys =
 *         GI->GetSubsystem<UInoChatterboxTurboNativeSubsystem>();
 *
 * Owns:
 *   - The loaded FInoChatterboxTurboNativeModels (four ORT sessions — speech_encoder,
 *     embed_tokens, language_model, conditional_decoder)
 *   - The loaded FInoChatterboxTurboNativeTokenizer (GPT-2 BPE + paralinguistic
 ...
 */
UCLASS()
class INOCHATTERBOXNATIVE_API UInoChatterboxTurboNativeSubsystem : public UGameInstanceSubsystem
{
    GENERATED_BODY()
...
public:
    ...
    UInoChatterboxTurboNativeSubsystem(FVTableHelper& Helper);
    virtual ~UInoChatterboxTurboNativeSubsystem();
     ...
     */
    UFUNCTION(BlueprintPure, Category = "InoAgents|Chatterbox")
    bool IsModelDownloaded(EInoChatterboxTurboNativeVariant Variant) const;
     ...
     */
    UFUNCTION(BlueprintPure, Category = "InoAgents|Chatterbox")
    bool IsConfigDownloaded(const FInoChatterboxTurboNativeModelConfig& Config) const;
     ...
     */
    UFUNCTION(BlueprintPure, Category = "InoAgents|Chatterbox")
    EInoChatterboxTurboNativeVariant GetLoadedVariant() const;
     ...
     * See FInoChatterboxTurboNativeVoice's header for the full priority list.
     ...
     *     FInoChatterboxTurboNativeRunner::SynthesizeText's OutError)
     *
     * MUST be called on the game thread.
     */
    UFUNCTION(BlueprintCallable, Category = "InoAgents|Chatterbox",
...
private:
    ...
    TUniquePtr<FInoChatterboxTurboNativeTokenizer> Tokenizer;

    /** Which variant LoadModelsAsync was called with. Only meaningful
     *  when Models.IsValid(). */
    EInoChatterboxTurboNativeVariant LoadedVariant = EInoChatterboxTurboNativeVariant::Q4F16;
    ...
    /** The dedicated worker thread that runs FInoChatterboxTurboNativeRunner on
    ...
    TUniquePtr<FInoChatterboxTurboNativeSynthesisWorker> Worker;
     ...
     *  CleanupDownload. Struct type FInoChatterboxTurboNativeDownloadFile is
    ...
    TArray<FInoChatterboxTurboNativeDownloadFile> DownloadQueue;
    ...
    FInoChatterboxTurboNativeModelConfig PendingConfig;
    ...
    FOnInoChatterboxTurboNativeModelsLoaded PendingOnLoaded;
    ...
    FOnInoChatterboxTurboNativeDownloadProgress PendingOnDownloadProgress;

    /** Download-flow helpers — see InoChatterboxTurboNativeSubsystem.cpp for the
     *  narrative; flow is:
     ...
     *  + directory. */
    void DispatchLoadWorker(const FInoChatterboxTurboNativeModelConfig& Config);
